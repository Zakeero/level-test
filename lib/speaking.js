...
async function gemini(parts, { json = true, temperature = 0.2, timeoutMs = 25_000 } = {}) {
  ...
  const models = [MODEL, MODEL, ...(FALLBACK_MODEL ? [FALLBACK_MODEL] : [MODEL]), MODEL];
  const deadline = Date.now() + 48_000; // serverless funksiya chegarasidan (60s) oshib ketmaslik uchun
  let last;
  for (let i = 0; i < models.length; i++) {
    if (Date.now() > deadline) break;
    try {
      const text = await geminiOnce(models[i], body, timeoutMs);
      ...
      if (!e.retryable && !timedOut) throw e;              // 400/401/404 — qayta urinishdan foyda yo'q
      if (i === models.length - 1 || Date.now() > deadline) break;
      await new Promise((s) => setTimeout(s, RETRY_DELAYS[Math.min(i, RETRY_DELAYS.length - 1)]));
    }
  }
  throw last;
}
